...
async def test_new_shard_download():
  ...
  shard_downloader.on_progress.register("test").on_next_async(
    lambda shard, event: asyncio.create_task(print_progress(shard, event))
  )
  
  async def print_progress(shard, event):
    print(shard, event)
